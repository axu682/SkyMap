...
class Star():
    ...
    def draw(self, data, canvas):
        if not (self.familyVisible and self.constellationVisible and self.manualVisible and self.positionVisible and self.horizonVisible):
            return
        if data.theme=="dark" and data.colorfulStars==True: color = self.color
        elif data.theme=="dark": color = "white"
        else: color = "black"
