...
def init(data):
    ...
    data.currentTimeAndDate = TimeAndDate(2019,7,31, 0,37,0)
    ...
    data.latitude = 42.2959
    data.longitude = -71.2128
    ...
    dimensionData.originCorner1 = (270,0)
    dimensionData.originCorner2 = (300,30)
    ...
    data.box = InputNumberBox(0, 24, True, False, 20, "positiveInt", "hour", dimensionData)
...
def keyPressed(event, data):
    ...
    if data.mode=="inputNumberBox":
        data.eventDelay = 0
        data.box.keyPressed(event, data)
    elif data.mode=="sky":
        if event.keysym=="u":
...
def redrawAll(canvas, data):
    if data.mode=="inputNumberBox":
        data.box.redrawAll(canvas, data)
    elif data.mode=="sky":
        printStatus(data)
        ...
        canvas.create_text((0,0), anchor="nw", text=getStatus(data), font="Courier 10", fill="white")
